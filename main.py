...
def trading_radar_loop(control_panel, signal_engine, execution_engine, monitored_assets):
    ...
    while True:
        try:
            # ✅ الحل: نعرف المتغير هنا داخل الـ try كل مرة، مستحيل يضرب not defined
            scalp_active = getattr(control_panel, 'scalp_mode_active', False)
            execution_engine.is_scalp_active = scalp_active
            ...
            if is_active:  
                ...
                mock_smc_data = {
                    ...
                    'is_scalping_signal': scalp_active 
                }
                ...
                if decision.get('status') == 'TRIGGERED':
                    try:
                        decision['trade_style'] = "⚡ SCALPING (خاطفة)" if scalp_active else "🏆 SWING (موجية)"
                    ...
                    style_label = "سكالبينج" if scalp_active else "طويلة الأمد"
